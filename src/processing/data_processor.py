...
class TextureDataProcessor:
    ...
    def process_texture_images_v2(self, texture_classes=None):
        """处理纹理图像v2 - 使用不同的噪声参数创建增强数据集"""
        if texture_classes is None:
            texture_classes = ["D1", "D2", "D3", "D4", "D5", "D6"]

        # 创建v2数据目录
        v2_processed_dir = os.path.join(self.base_dir, "processed_v2")
        if not os.path.exists(v2_processed_dir):
            os.makedirs(v2_processed_dir)
            print(f"创建v2主目录: {v2_processed_dir}")

        for class_name in texture_classes:
            print(f"处理类别 v2: {class_name}")

            # 创建v2训练和测试目录
            train_dir = os.path.join(v2_processed_dir, f"{class_name}_train")
            test_dir = os.path.join(v2_processed_dir, f"{class_name}_test")
                ...
                if len(image_files) >= 16:
                    ...
                    for i, filename in enumerate(train_files):
                        ...
                        try:
                            ...
                            # v2: 训练集也添加轻微噪声，增强模型鲁棒性
                            noisy_image = add_salt_pepper_noise(
                                image, salt_prob=0.001, pepper_prob=0.001
                            )
                    ...
                    for i, filename in enumerate(test_files):
                        ...
                        try:
                            ...
                            # v2: 更强的噪声，测试模型在恶劣条件下的表现
                            noisy_image = add_salt_pepper_noise(
                                image, salt_prob=0.01, pepper_prob=0.01  # 增加噪声强度
                            )
                            ...
                            print(f"  v2添加噪声: {class_name}_{i+1}.png")

                        except Exception as e:
                            print(f"处理测试图像 {filename} 时出错: {e}")

                    print(f"  v2成功处理: {train_count}张训练 + {test_count}张测试")
